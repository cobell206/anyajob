...
- 2026-07-11 — **weekly fix verified + email feature retired (user: unused).**
  After the store-seam fix, `weekly` read S3 + generated the reflection, then SES
  rejected the send because the live prefs recipient is the placeholder
  `"FILL IN — her email address"` (spaces → "Local address contains control or
  whitespace"). `resolveRecipients` doesn't validate, and **`daily`'s morning
  email uses the same S3 recipient** — so morning emails have silently failed on
  prod since M3.5 (only EC2 weekly "worked", via the frozen-local-prefs bug).
  User: email unused → ignore. Cleanup: **dropped the `CronWeekly` schedule**
  (weekly is purely email; the reflection is still generated by daily on
  Sundays) — 2 schedules remain (daily, discover); `notifications.to` cleared to
